...
import {
  ResponsiveContainer, PieChart, Pie, Cell, Tooltip, Legend, Bar, XAxis, YAxis, ComposedChart, Line, BarChart
} from "recharts";
...
const formatChartData = (data: { _id: string | null; count: number }[] | undefined): { name: string; count: number }[] => {
    return data?.map(item => ({ name: (item._id || 'N/A').replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase()), count: item.count })) || [];
};
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-600 dark:text-slate-400 mt-1">An overview of key metrics and data trends across the platform.</p>
      </div>

      {/* ✅ User Statistics Cards with new background colors */}
      <div>
        <h2 className="text-xl font-semibold mb-4 flex items-center"><Users className="w-5 h-5 mr-2 text-primary" /> User Statistics</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <StatCard title="Total Users" value={userStats?.totalUsers ?? 0} icon={Users} details={`+${userStats?.newUsersInLast30Days ?? 0} in 30 days`} 
            className="bg-blue-100 dark:bg-blue-950 text-blue-800 dark:text-blue-200" />
          <StatCard title="Active Users" value={userStats?.totalActiveUsers ?? 0} icon={UserCheck} details={`+${userStats?.newUsersInLast7Days ?? 0} this week`} 
            className="bg-green-100 dark:bg-green-950 text-green-800 dark:text-green-200" />
          <StatCard title="Inactive Users" value={userStats?.totalInActiveUsers ?? 0} icon={UserX} 
            className="bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-200" />
          <StatCard title="Blocked Users" value={userStats?.totalBlockedUsers ?? 0} icon={UserX} 
      ...
      <div>
        <h2 className="text-xl font-semibold mb-4 flex items-center"><Package className="w-5 h-5 mr-2 text-primary" /> Parcel Statistics</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <StatCard title="Total Parcels" value={parcelStats?.totalParcel ?? 0} icon={Package} details={`+${parcelStats?.parcelCreatedInLast30Days ?? 0} in 30 days`} 
            className="bg-violet-100 dark:bg-violet-950 text-violet-800 dark:text-violet-200" />
          <StatCard title="Avg. Fee" value={`৳${(parcelStats?.avgFeePerParcel?.[0]?.avgFee || 0).toFixed(2)}`} icon={CircleDollarSign} details="Per parcel" 
            className="bg-cyan-100 dark:bg-cyan-950 text-cyan-800 dark:text-cyan-200" />
          <StatCard title="Unique Senders" value={parcelStats?.totalParcelCreatedByUniqueSender ?? 0} icon={Users} 
            className="bg-sky-100 dark:bg-sky-950 text-sky-800 dark:text-sky-200" />
          <StatCard title="Unique Receivers" value={parcelStats?.totalParcelReceiverByUniqueReceiver ?? 0} icon={Users} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
              <CardHeader>
                <CardTitle className="flex items-center"><PieIcon className="w-5 h-5 mr-2" />User Role Distribution</CardTitle>
                <CardDescription>A breakdown of all users by their assigned roles.</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
              <CardHeader>
                <CardTitle className="flex items-center"><BarChart2 className="w-5 h-5 mr-2" />Parcel Status</CardTitle>
                <CardDescription>The current status distribution of all parcels.</CardDescription>
              </CardHeader>
              <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
          ...
          <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
              <CardHeader>
                <CardTitle className="flex items-center"><LineChart className="w-5 h-5 mr-2" />New Parcels Trend</CardTitle>
                <CardDescription>Number of new parcels created over time.</CardDescription>
              </CardHeader>
              <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
          ...
          <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            <CardHeader>
              ...
              <CardDescription>Further breakdown of parcels by type and shipping method.</CardDescription>
            </CardHeader>
            <CardContent>
                <Tabs defaultValue="type">
                    <TabsList className="grid w-full grid-cols-2"><TabsTrigger value="type">By Type</TabsTrigger><TabsTrigger value="shipping">By Shipping Method</TabsTrigger></TabsList>
